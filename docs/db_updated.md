...
CREATE TABLE posture_daily_report (
    ...
    score                     SMALLINT,                -- 자세 점수
    correct_posture_ratio     REAL,                    -- 정자세 비율 0~1
    total_sitting_s           INTEGER,                 -- 총 착석 시간(초)
    max_continuous_sitting_s  INTEGER,                 -- 최대 연속 착석(초)
    turtle_neck_count         INTEGER,                 -- 거북목 감지 횟수
    alert_accept_ratio        REAL,                    -- 알림 수락률 0~1
...
CREATE TABLE posture_hourly_stat (
    id                INTEGER PRIMARY KEY,
    report_id         INTEGER NOT NULL,    -- posture_daily_report.id
    hour              VARCHAR(2) NOT NULL, -- '00'~'23'
    score             SMALLINT,
    turtle_neck_count SMALLINT,
...
CREATE TABLE posture_report (
    ...
    daily_report_id INTEGER,                 -- daily 한정, posture_daily_report.id
    metrics         TEXT,                    -- json: 구조화 지표
    report_text     TEXT,                    -- LLM 생성 리포트(agent api.md §1.2)

    CHECK (period IN ('daily', 'weekly')),
    UNIQUE (user_id, period, period_start),
    FOREIGN KEY (user_id) REFERENCES user(id),
    FOREIGN KEY (daily_report_id) REFERENCES posture_daily_report(id)
);
...
- 반복 루틴(`routine_task`)과 특정 날짜 1회성 일정(`event`)은 성격이 달라 별도 테이블로 둔다. 둘 다 요일/카테고리/시간대라는 같은 shape을 공유하되, 반복 기준만 `day_of_week`(루틴) vs `event_date`(1회성)로 다르다.
...
CREATE TABLE event (
    ...
    created_by   VARCHAR(10)  NOT NULL,   -- 'user' | 'agent' 생성 주체
    category     VARCHAR(10)  NOT NULL,   -- 예: 'posture' | 'sleep' | 'diet' | 'mental' | ...
    event_date   VARCHAR(10)  NOT NULL,   -- 1회성 일정 날짜 'YYYY-MM-DD' (day_of_week 대신)
    start_minute INTEGER,                 -- 자정 기준 분(0~1440). 시간 미지정이면 NULL
    end_minute   INTEGER,                 -- 자정 기준 분(0~1440)
    done         INTEGER      NOT NULL,   -- 0 = 미완료, 1 = 완료
...
CREATE INDEX idx_event_user_date ON event (user_id, event_date);
```

`GET/PATCH .../routine-tasks`(agent API §2.4/2.5)는 이 두 테이블을 `type` 필드로 구분해 하나의 API로 함께 다룬다(엔드포인트를 늘리지 않음). `id`는 테이블별로 독립된 PK 시퀀스이므로 두 테이블 사이에서 값이 겹칠 수 있어, `taskId`만으로는 대상 테이블을 특정할 수 없다 — 반드시 `type`을 함께 사용해 구분한다.
